...
#
# The number has to sit in the label itself, not merely somewhere after it: CCM deletes
# -x86_64/-aarch64 wherever it sits before parsing (ccmlib/utils/version.py:49-50), so
# 6.2.0-dev-aarch64 would reach it as the bare, moving selector 6.2.0-dev, resolved to
# whichever build is newest today. A label carrying no number of its own is exact only
# when a dated build id follows it. With no label at all, a dated id
# (5.1.2-0.20221225.4c0f7ea09893) is not merely inexact: normalize_scylla_version rewrites
# its '-' to '~' and CCM then matches none.
